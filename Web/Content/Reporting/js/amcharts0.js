        ...
        parseDatesDraw: function() {
            ...
            if (0 < this.gridCountR)
                for (B - 5 - k > this.autoRotateCount && !isNaN(this.autoRotateAngle) && (this.labelRotationR = this.autoRotateAngle), a = k; a <= B; a++) {
                    ...
                    m = l = p;
                    if (!isNaN(u))
                        for (h = 1; h < x; h += u) this.gridAlpha = this.minorGridAlpha, f = p + y * h, f = d.resetDateToMin(new Date(f), A, u, t).getTime(), f = new this.axisItemRenderer(this, (f - this.startTime) * this.stepWidth, void 0, void 0, void 0, void 0, void 0, void 0, void 0, !0), this.pushAxisItem(f);
                    this.gridAlpha = w
                }
        },
    ...
    d.ValueAxis = d.Class({
        ...
        draw: function() {
            ...
            if (isNaN(this.min) || isNaN(this.max) || !this.foundGraphs || Infinity == this.min || -Infinity == this.max) e = !0;
                ...
                this.exponential = !1;
                for (g = t; g < r; g += p) {
                    var x = d.roundTo(this.step * g + this.min, w); - 1 != String(x).indexOf("e") && (this.exponential = !0)
                }
                this.duration && (this.maxInterval = d.getMaxInterval(this.max, this.duration));
                var w = this.step,
                    A, x = this.minorGridAlpha;
                ...
                if (this.autoGridCount || 0 !== this.gridCount)
                    if ("date" == c) this.generateDFObject(), this.timeDifference = this.max - this.min, this.maxTime = this.lastTime = this.max, this.startTime = this.firstTime = this.min, this.parseDatesDraw();
                        ...
                        for (r >= this.autoRotateCount && !isNaN(this.autoRotateAngle) && (this.labelRotationR = this.autoRotateAngle), h && r++, g = t; g < r; g += p)
                            if (c = w * g + this.minCalc, c = d.roundTo(c, this.maxDecCount + 1), !this.integersOnly || Math.round(c) == c)
...
(function() {
    ...
    d.AmChart = d.Class({
        construct: function(a) {
            ...
            this.color = this.borderColor = "#fff";					//color of text in chart
        ...
        brr: function() {
            if (!this.hideCredits) {
                ...
                if (c != a || !0 === this.inIframe()) {
                    ...
                    g.color = this.color;
                    g.fontFamily = "Tahoma";   // or: this.fontFamily
                    g.fontSize = "11px";
                    g.opacity = .7;
                    g.display = "block";
                    var k = a.offsetWidth,
    ...
    d.AmGraph = d.Class({
        ...
        createSerialGraph: function() {
            ...
            var m = this.useNegativeColorIfDown,
                ...
                y = [],
                x = [],
                A, z, B, C, D = this.chart.graphs.length,
            ...
            for (K = Q; K <= ia; K++) {
                ...
                if (Ia) {
                    ...
                    switch (g) {
                        case "line":
                            if (isNaN(W)) w || (this.drawLineGraph(y, x, U, V), y = [], x = [], U = [], V = []);
                            else {
                            ...
                            break;
                        case "smoothedLine":
                            if (isNaN(W)) w || (this.drawSmoothedGraph(y, x, U, V), y = [], x = [], U = [], V = []);
                            else {
                                if (Math.abs(E - jb) >= lb || Math.abs(F - kb) >= lb) y.push(E), x.push(F), jb = E, kb = F;
                                wa = E;
                                Da = F;
...
(function() {
    ...
    d.wedge = function(a, b, c, e, h, f, g, k, l, m, n, q, p, t) {
        ...
        var x = 1 / 180 * Math.PI,
            ...
            z = b + Math.sin(e * x) * f,
            B = c - Math.cos(e * x) * g,
            C = b + Math.sin((e + h) * x) * f,
            D = c - Math.cos((e + h) * x) * g,
            L = b + Math.sin((e + h) * x) * k,
            x = c - Math.cos((e + h) * x) * u,
            I = {
